...
def convert_to_pix(label):
    ...
    half_x_H = x_label*monitor_width/2 * width/monitor_width
    half_y_H = y_label*monitor_height/2 * height/monitor_height

    x_visual_angle = 2 * torch.atan(half_x_H/D) * 180 / 3.1416
    y_visual_angle = 2 * torch.atan(half_y_H/D) * 180 / 3.1416

    x_visual_angle = x_visual_angle.unsqueeze(1)
    y_visual_angle = y_visual_angle.unsqueeze(1)
...
def trainer(net,train_loader,optimizer, epoch, loss_func):
    ...
    for id, (img, labels,centroid,bbox) in enumerate(train_loader):
        ...
        img = img.unsqueeze(1).cuda(device=3)
        #result = net(img,centroid,bbox)
        result = net(img)
        labels = convert_to_pix(labels)
        labels = labels.cuda(device=3)
...
def valer(net,val_loader,loss_func):
    ...
    with torch.no_grad():
        ...
        for id, (img, labels, centroid, bbox) in enumerate(val_loader):
            ...
            #bbox = bbox.view(bbox.size(0), 12).cuda()/84.0
            img = img.unsqueeze(1)
            img = img.cuda(device=3)
            #out = net(img,centroid,bbox)
            out = net(img)
            labels = convert_to_pix(labels)
            labels = labels.cuda(device=3)
            loss = loss_func(out,labels[0,:])
            sum_loss += loss.item()
        sum_loss = sum_loss/id
